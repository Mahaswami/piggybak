...
var piggybak_states = {
	...
	update_state_option: function(type, block) {
		...
		if( geodata.countries["country_" + country_id] && geodata.countries["country_" + country_id].length > 0) {
			new_field = $('<select>');
		...
		if (typeof(old_field.attr('name')) != "undefined") {
		    new_field.attr('name', old_field.attr('name'));
		}
        if (typeof(old_field.attr('id')) != "undefined") {
